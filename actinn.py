...
def scale_sets(sets):
    ...
    total_set = total_set[np.logical_and(expr >= np.percentile(expr, 1),
                                         expr <= np.percentile(expr, 99)), ]
    cv = np.std(total_set, axis=1) / np.mean(total_set, axis=1)
    total_set = total_set[np.logical_and(cv >= np.percentile(cv, 1), cv <= np.
                                         percentile(cv, 99)), ]
    for i in range(len(sets)):
        sets[i] = total_set[:,
...
for sample in samples:
    train_set = read_data(path_in + sample + '_train.txt')
    test_set = read_data(path_in + sample + '_test.txt')
    metric_res[sample] = main(train_set, test_set)

with open(path_out + tissue + '_actinn.pkl', 'wb') as f:
...
summary['Method'] = ['ACTINN'] * len(metric_res) * 2
summary['Metric'] = ['Acc', 'MacroF1'] * len(metric_res)
pd.DataFrame(summary).to_csv(path_out + tissue + '_actinn_res.txt',
                             sep='\t',
                             index=False)
